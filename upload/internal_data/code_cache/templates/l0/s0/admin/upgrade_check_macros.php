...
return array('macros' => array('serious_errors' => function($__templater, array $__arguments, array $__vars)
{
	...
	if (strlen(trim($__compilerTemp1)) > 0) {
		...
		<div class="blockMessage blockMessage--error blockMessage--iconic">
			' . 'The following issues must be resolved as soon as possible. Once they have been resolved, <a href="' . $__templater->fn('link', array('tools/upgrade-check', ), true) . '">check for upgrades again</a> to confirm.' . '
			<ul>
			' . $__compilerTemp1 . '
			</ul>
		</div>
...
'warnings' => function($__templater, array $__arguments, array $__vars)
{
	...
	if (!$__templater->method($__vars['upgradeCheck'], 'hasLicenseErrors', array())) {
		...
		if (!$__vars['upgradeCheck']['board_url_valid']) {
			$__compilerTemp1 .= '
						<li>' . 'The <a href="' . ($__templater->fn('link', array('options/groups/basicBoard/', ), true) . '#boardUrl') . '">Board URL option</a> does not match the site URL configured for your license.' . '
						</li>
					';
		}
		$__compilerTemp1 .= '
		...
		if (strlen(trim($__compilerTemp1)) > 0) {
			...
			<div class="blockMessage blockMessage--warning blockMessage--iconic">
				' . 'The following issues should be resolved. Once they have been resolved, <a href="' . $__templater->fn('link', array('tools/upgrade-check', ), true) . '">check for upgrades again</a> to confirm.' . '
				<ul>
				' . $__compilerTemp1 . '
				</ul>
			</div>
		...
		if ($__templater->method($__vars['upgradeCheck'], 'hasAvailableUpdate', array('XF', ))) {
			$__compilerTemp1 .= '
						...
						<div style="margin-top: .5em">
							...
							', array(
				'href' => $__templater->fn('link', array('tools/upgrade-xf', ), false),
				'class' => 'button--link',
			), '', array(
			)) . '
						</div>
			...
			$__compilerTemp1 .= '
						...
						<div style="margin-top: .5em">
							' . $__templater->button('
								' . 'Upgrade now' . '
							', array(
				'href' => $__templater->fn('link', array('tools/upgrade-xf-add-on', ), false),
				'class' => 'button--link',
			), '', array(
			)) . '
						</div>
