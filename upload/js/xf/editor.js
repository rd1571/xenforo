	...
	XF.Editor = XF.Element.newHandler({
		...
		editorInit: function()
		{
			...
			ed.events.on('paste.beforeCleanup', function(content)
			{
				...
				// sometimes URLs are auto-linked when pasting. in IOS this seems to be any link. in Chrome desktop
				// this appears to be any URL copied from the address bar. because this interferes with unfurling
				// (an already linked URL cannot be unfurled) attempt to detect and extract the links to paste as text.
				var match;
				if (match = content.match(/^(?:<meta charset=(?:'|")[^'"]*(?:'|")>)?<a href=(?:'|")([^'"]*)(?:'|")>\1<\/a>$/))
				{
